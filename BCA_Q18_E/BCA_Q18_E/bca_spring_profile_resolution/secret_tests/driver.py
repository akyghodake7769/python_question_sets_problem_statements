...
def run_tests():
    ...
    prop_path = os.path.join(get_base_path(), 'application.properties')
    if os.path.exists(prop_path):
        with open(prop_path, 'r') as f:
            code = f.read()
        results['tc1'] = True
        results['tc2'] = ('active=prod' in code.replace(' ', '') or 'active=production' in code.replace(' ', ''))
        results['tc3'] = ('db' in code or 'url' in code)
        results['tc4'] = ('password' in code)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == '--json':
        ...
        TC_NAMES = {
            "tc1": "application.properties has valid overrides",
            "tc2": "Dev properties overridden correctly",
            "tc3": "Prod connection profiles configured",
            "tc4": "Security password configurations locked down"
        }
        print("Running Auto-Evaluation for: Basic Code Analysis: Debugging Active Profile Property Overrides\n")
        total_score = 0
