...
def run_tests():
    ...
    py_path = os.path.join(get_base_path(), 'cache_service.py')
    if os.path.exists(py_path):
        with open(py_path, 'r') as f:
            code = f.read()
        results['tc1'] = True
        results['tc2'] = ('delete' in code or 'invalidate' in code or 'expire' in code)
        results['tc3'] = ('product' in code or 'key' in code)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == '--json':
        ...
        TC_NAMES = {
            "tc1": "cache_service.py exists in student_workspace/ with valid Python code",
            "tc2": "Cache deletion / invalidation call added",
            "tc3": "Key pattern 'product:{id}' invalidated"
        }
        print("Running Auto-Evaluation for: Basic Code Analysis: Cache Invalidation & TTL Eviction Logic\n")
        total_score = 0
