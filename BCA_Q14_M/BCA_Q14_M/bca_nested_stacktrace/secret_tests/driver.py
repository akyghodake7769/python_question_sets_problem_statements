...
def run_tests():
    ...
    exc = data.get('root_exception', '').strip().lower()
    code = int(data.get('sql_error_code', 0))
    cls = data.get('blocking_class', '').strip().lower()
    results['tc1'] = True
    results['tc2'] = ('sqlexception' in exc)
    results['tc3'] = (code == 1205)
    results['tc4'] = ('transactionmanager' in cls or 'payment' in cls)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == '--json':
        ...
        print("Running Auto-Evaluation for: Basic Code Analysis: Complex Nested Stack Trace Diagnostics\n")
        total_score = 0
