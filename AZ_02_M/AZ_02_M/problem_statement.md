# DevOps Lab: Azure Virtual Machine & Security Configuration
...
- **Resource Group:** Use the pre-existing Resource Group `iRun-Assessment-test`.
...
- **Network Security Group Name:** `nsg-<username>`
...
- **Network Association:** Associate the VM's network interface with `vnet-<username>`, subnet `default`, and bind it to the Network Security Group `nsg-<username>`. The VM must be in a running state.
...
| **TC2**   | Virtual Network (`vnet-<username>` 10.0.0.0/16) and subnet default (`10.0.0.0/24`) in `eastasia` | 4 Marks |
| **TC3**   | NSG (`nsg-<username>`) exists and is associated | 4 Marks |
| **TC4**   | NSG rules check (Inbound SSH traffic allowed on Port 22) | 4 Marks |
| **TC5**   | Ubuntu VM (`vm-<username>` Standard_B1s) is deployed and running | 4 Marks |
