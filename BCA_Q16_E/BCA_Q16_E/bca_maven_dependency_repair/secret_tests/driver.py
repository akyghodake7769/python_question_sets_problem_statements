...
def run_tests():
    ...
    xml_path = os.path.join(get_base_path(), 'pom.xml')
    if os.path.exists(xml_path):
        with open(xml_path, 'r') as f:
            code = f.read()
        results['tc1'] = ('<project' in code)
        results['tc2'] = ('<exclusion>' in code and 'jackson' in code)
        results['tc3'] = ('</exclusion>' in code)
        results['tc4'] = ('<dependencies>' in code)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == '--json':
        ...
        TC_NAMES = {
            "tc1": "pom.xml contains valid XML",
            "tc2": "Conflicting jackson library correctly excluded",
            "tc3": "Correct exclusion schema syntax used",
            "tc4": "Main dependencies remain intact"
        }
        print("Running Auto-Evaluation for: Basic Code Analysis: Repairing Maven Dependency Conflict Warnings\n")
        total_score = 0
