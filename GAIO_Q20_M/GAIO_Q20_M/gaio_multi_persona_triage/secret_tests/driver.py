...
def run_tests():
    ...
    dba = data.get('dba_focus', '').strip().lower()
    sre = data.get('sre_focus', '').strip().lower()
    synth = data.get('synthesis_rule', '').strip().lower()
    results['tc1'] = True
    results['tc2'] = ('lock' in dba or 'connection' in dba or 'pool' in dba or 'sql' in dba)
    results['tc3'] = ('latency' in sre or 'packet' in sre or 'network' in sre or 'bandwidth' in sre)
    results['tc4'] = (len(synth) > 10)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == '--json':
        ...
        TC_NAMES = {
            "tc1": "multi_persona_config.json exists",
            "tc2": "DBA persona focuses on locks and connection pools",
            "tc3": "Network SRE persona focuses on latency and packet loss",
            "tc4": "Consolidated RCA synthesis section defined"
        }
        print("Running Auto-Evaluation for: GenAI Ops: Multi-Persona Incident Triage Prompting\n")
        total_score = 0
