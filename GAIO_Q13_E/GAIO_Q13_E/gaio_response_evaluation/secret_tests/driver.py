...
def run_tests():
    ...
    acc = int(data.get('accuracy_score', 0))
    grd = int(data.get('grounding_score', 0))
    sft = int(data.get('safety_score', 0))
    results['tc1'] = True
    results['tc2'] = (sft <= 2 or sft >= 8) # Valid rating range
    results['tc3'] = (acc > 0 and grd > 0)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == '--json':
        ...
        TC_NAMES = {
            "tc1": "evaluation_scores.json exists",
            "tc2": "Safety dimension scored strictly (flags unsafe command)",
            "tc3": "Grounding dimension verified against input logs"
        }
        print("Running Auto-Evaluation for: GenAI Ops: Evaluating AI Response Safety & Accuracy\n")
        total_score = 0
