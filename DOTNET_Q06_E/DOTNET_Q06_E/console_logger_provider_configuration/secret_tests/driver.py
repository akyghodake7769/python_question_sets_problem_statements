...
def run_tests():
    ...
    try:
        with open(app_path, 'r') as f:
            data = json.load(f)
        default_log = data.get("Logging", {}).get("Console", {}).get("LogLevel", {}).get("Default", "")
        if default_log.lower() == "debug":
            results['tc3'] = True
    except Exception:
        pass
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == '--json':
        ...
        TC_NAMES = {
            ...
            "tc2": "File appsettings.json exists and is valid JSON", "tc3": "Logging:Console:LogLevel:Default is set to Debug"
        }
        print("Running Tests for: Console Logger Provider Configuration\n")
        total_score = 0
        marks_list = [5, 5]
