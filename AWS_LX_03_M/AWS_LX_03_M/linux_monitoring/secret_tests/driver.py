...
def verify_task():
    ...
    print(f"TC2: {'CPU/process logs generated':<30} [{'PASSED' if tc2_passed else 'FAILED'}] ({5 if tc2_passed else 0}/5)")
    ...
    print(f"TC3: {'Memory logs generated':<30} [{'PASSED' if tc3_passed else 'FAILED'}] ({5 if tc3_passed else 0}/5)")

    # TC4
    tc4_passed = False
    if tc1_passed:
        out = run_ssm("test -s /home/ubuntu/disk.txt && test -s /home/ubuntu/network.txt && echo PASS || echo FAIL")
        if out and "PASS" in out: tc4_passed = True
    results['tc4'] = tc4_passed
    total_score += 5 if tc4_passed else 0
    print(f"TC4: {'Disk/Network logs generated':<30} [{'PASSED' if tc4_passed else 'FAILED'}] ({5 if tc4_passed else 0}/5)")
