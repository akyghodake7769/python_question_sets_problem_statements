...
def run_tests():
    ...
    rb1 = data.get('db_timeout_runbook', '').strip().upper()
    rb2 = data.get('memory_leak_runbook', '').strip().upper()
    results['tc1'] = True
    results['tc2'] = ('RB-102' in rb1 or '102' in rb1)
    results['tc3'] = ('RB-205' in rb2 or '205' in rb2)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == '--json':
        ...
        TC_NAMES = {
            ...
            "tc2": "Database connection pool timeout mapped to RB-102",
            "tc3": "Memory leak exception mapped to RB-205"
        }
        print("Running Auto-Evaluation for: Log Analysis: Error Signatures & Runbook Mapping\n")
        total_score = 0
