import boto3, os, sys, json, base64
from datetime import datetime, timezone

# Capture Assessment Start Time
START_TIME_STR = os.getenv('KODEARENA_START_TIME') or os.getenv('KLOUDKRAFT_START_TIME')
...
def verify_task():
    ...
    username = os.getenv('KODEARENA_USERNAME') or os.getenv('KLOUDKRAFT_USERNAME', USER_PREFIX)
    ...
    try:
        ...
        print(f"[SYSTEM] Validating Resources for: {username}")
        print(f"[SYSTEM] Session Active Time: {elapsed_minutes:.1f} mins\n")

        try:
            s3 = boto3.client('s3', region_name='eu-west-2')
        except Exception as e:
            print(f"[ERROR] AWS Connection Failed: {e}"); return
        ...
        for r in candidate_regions:
            try:
                dynamodb = boto3.client('dynamodb', region_name=r)
        ...
        if desc:
            ...
            print(f"     └─ [Reason]: DynamoDB table '{table_name}' was not found in checked regions {candidate_regions}. Last error: {last_err}")
            results['tc4'] = False
            print(f"TC5: DynamoDB Partition Key is LockID ........................... [FAILED] (0/3)")
        ...
        if not tf_executed:
            ...
            if os.path.exists(os.path.join(parent_dir, '.terraform')) or os.path.exists(os.path.join(parent_dir, '.terraform.lock.hcl')) or os.path.exists(os.path.join(parent_dir, 'terraform.tfstate')):
                tf_executed = True
        ...
        if not tf_executed:
            # If resources are created, Terraform must have executed!
            if tc1_passed or tc4_passed:
                tf_executed = True
