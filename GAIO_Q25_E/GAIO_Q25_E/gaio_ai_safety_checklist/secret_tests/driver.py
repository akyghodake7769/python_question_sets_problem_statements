...
def run_tests():
    ...
    py_path = os.path.join(get_base_path(), 'evaluate_safety.py')
    if os.path.exists(py_path):
        with open(py_path, 'r') as f:
            code = f.read()
        results['tc1'] = True
        results['tc2'] = ('safety' in code.lower() and 'grounding' in code.lower())
        results['tc3'] = ('score' in code.lower() or 'total' in code.lower())
        results['tc4'] = ('safety_score.json' in code)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == '--json':
        ...
        TC_NAMES = {
            "tc1": "evaluate_safety.py is syntax-valid Python",
            "tc2": "Evaluates Accuracy, Grounding, Completeness, Format, Actionability, Safety",
            "tc3": "Calculates composite safety score out of 100",
            "tc4": "Outputs safety_score.json"
        }
        print("Running Auto-Evaluation for: GenAI Ops: AI Safety Checklist & Grounding Score Calculator\n")
        total_score = 0
