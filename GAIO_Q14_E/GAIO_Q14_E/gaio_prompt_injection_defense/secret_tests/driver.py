...
def run_tests():
    ...
    txt = data.get('clean_prompt_input', '').strip().lower()
    results['tc1'] = True
    results['tc2'] = ('ignore previous' not in txt and 'print secrets' not in txt)
    results['tc3'] = ('error' in txt or 'exception' in txt or len(txt) > 20)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == '--json':
        ...
        TC_NAMES = {
            "tc1": "clean_input.json exists",
            "tc2": "Prompt injection payload ('Ignore previous rules') stripped",
            "tc3": "Legitimate error log message preserved"
        }
        print("Running Auto-Evaluation for: GenAI Ops: Prompt Injection & Log Poisoning Defense\n")
        total_score = 0
