...
def run_tests():
    ...
    re_str = data.get('refactored_regex', '').strip()
    results['tc1'] = True
    results['tc2'] = ('(a+)+' not in re_str and '(.*)*' not in re_str)
    results['tc3'] = ('?' in re_str or '^' in re_str or '[^' in re_str)
    results['tc4'] = True
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == '--json':
        ...
        TC_NAMES = {
            "tc1": "safe_regex.json exists",
            "tc2": "Nested quantifier patterns (e.g. (a+)+) removed",
            "tc3": "Non-greedy or atomic group match syntax applied",
            "tc4": "Regex execution completes under 10ms"
        }
        print("Running Auto-Evaluation for: Log Analysis: Safe Regex Practices & Catastrophic Backtracking Prevention\n")
        total_score = 0
