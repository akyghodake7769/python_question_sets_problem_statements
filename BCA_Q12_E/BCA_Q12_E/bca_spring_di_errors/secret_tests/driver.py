...
def run_tests():
    ...
    java_path = os.path.join(get_base_path(), 'UserService.java')
    if os.path.exists(java_path):
        with open(java_path, 'r') as f:
            code = f.read()
        results['tc1'] = True
        results['tc2'] = ('@Service' in code or '@Component' in code)
        results['tc3'] = ('class UserService' in code)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == '--json':
        ...
        TC_NAMES = {
            "tc1": "UserService.java exists and is syntax-valid",
            "tc2": "@Service or @Component annotation added",
            "tc3": "@Autowired or constructor injection configured correctly"
        }
        print("Running Auto-Evaluation for: Basic Code Analysis: Resolving Spring Bean Creation Failures\n")
        total_score = 0
