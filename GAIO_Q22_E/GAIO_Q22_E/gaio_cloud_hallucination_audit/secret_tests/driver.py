...
def run_tests():
    ...
    tf = [x.strip().lower() for x in data.get('fake_tf_resources', [])]
    boto = [x.strip().lower() for x in data.get('fake_boto3_methods', [])]
    results['tc1'] = True
    results['tc2'] = any('super_bucket' in x for x in tf)
    results['tc3'] = any('delete_everything' in x for x in boto)
    results['tc4'] = not any(x == 'aws_s3_bucket' for x in tf)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == '--json':
        ...
        TC_NAMES = {
            "tc1": "cloud_audit.json exists",
            "tc2": "Fake Terraform resource 'aws_s3_super_bucket' flagged",
            "tc3": "Fake Boto3 method 'delete_everything()' flagged",
            "tc4": "Valid AWS resources ('aws_s3_bucket') preserved as legitimate"
        }
        print("Running Auto-Evaluation for: GenAI Ops: Imaginary Cloud Resource & Fake API Hallucination Audit\n")
        total_score = 0
