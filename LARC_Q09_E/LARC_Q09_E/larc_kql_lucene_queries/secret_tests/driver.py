...
def run_tests():
    ...
    kql = data.get('kql_query', '').strip().lower()
    results['tc1'] = True
    results['tc2'] = ('status' in kql and '502' in kql)
    results['tc3'] = ('2000' in kql and ('>' in kql or 'gt' in kql))
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == '--json':
        ...
        TC_NAMES = {
            ...
            "tc2": "KQL status code filter (status: 502) correctly constructed",
            "tc3": "Response time range condition (> 2000) included"
        }
        print("Running Auto-Evaluation for: Log Analysis: Enterprise KQL & Lucene Log Search Queries\n")
        total_score = 0
