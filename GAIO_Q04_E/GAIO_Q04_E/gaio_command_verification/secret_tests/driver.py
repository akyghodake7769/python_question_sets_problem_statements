...
def run_tests():
    ...
    s1 = data.get('rm_rf_safety', '').strip().upper()
    s2 = data.get('kubectl_get_safety', '').strip().upper()
    results['tc1'] = True
    results['tc2'] = (s1 == 'NEVER_RUN' or 'NEVER' in s1 or 'RISK' in s1)
    results['tc3'] = (s2 == 'SAFE')
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == '--json':
        ...
        TC_NAMES = {
            ...
            "tc2": "'rm -rf /' marked as NEVER_RUN",
            "tc3": "'kubectl get pods' marked as SAFE"
        }
        print("Running Auto-Evaluation for: GenAI Ops: Production Operational Verification & Safe Execution\n")
        total_score = 0
