...
def run_tests():
    ...
    if os.path.exists(prop_path):
        with open(prop_path, 'r') as f:
            lines = f.read()
        results['tc1'] = True
        results['tc2'] = ('spring.profiles.active=dev' in lines or 'spring.profiles.active = dev' in lines)
        results['tc3'] = ('3306' in lines)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == '--json':
        ...
        TC_NAMES = {
            "tc1": "application.properties has valid Spring boot keys",
            "tc2": "Active profile set to 'dev'",
            "tc3": "Database URL port set to local container port 3306"
        }
        print("Running Auto-Evaluation for: Basic Code Analysis: Auditing Spring properties files\n")
        total_score = 0
