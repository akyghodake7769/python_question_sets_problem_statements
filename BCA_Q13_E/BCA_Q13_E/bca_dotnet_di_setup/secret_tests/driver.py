...
def run_tests():
    ...
    cs_path = os.path.join(get_base_path(), 'Program.cs')
    if os.path.exists(cs_path):
        with open(cs_path, 'r') as f:
            code = f.read()
        results['tc1'] = True
        results['tc2'] = ('AddScoped<IDbService' in code.replace(' ', '') or 'AddScoped<IDbContext' in code.replace(' ', ''))
        results['tc3'] = ('AddTransient<IDbService' not in code.replace(' ', ''))
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == '--json':
        ...
        TC_NAMES = {
            "tc1": "Program.cs file exists",
            "tc2": "IDbService registered as AddScoped",
            "tc3": "Transient database configurations removed"
        }
        print("Running Auto-Evaluation for: Basic Code Analysis: ASP.NET Core Dependency Injection Lifetimes\n")
        total_score = 0
