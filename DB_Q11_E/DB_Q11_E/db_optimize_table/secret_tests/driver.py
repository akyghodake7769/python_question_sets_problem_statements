...
def verify_task():
    ...
    tc1_name = "TC1: Delta table compact files check"
    tc1_status = "[FAILED]"
    tc1_score = 0
    
    table_name = f"{username}_{exam_code}_optimize_table"
    table_obj = None
            ...
            try:
                for cat in ["kraft_data", "main", "hive_metastore"]:
                    for t in client.tables.list(catalog_name=cat, schema_name="default"):
                        if t.name.lower().endswith("_optimize_table"):
    ...
    if table_obj:
        ...
        if found_any_table:
            tc1_reason = f"Table '{table_name}' not found. But found '{found_any_table}'. Please check your prefix in solution.json."
        else:
            tc1_reason = f"Table '{table_name}' not found."

    tc2_name = "TC2: Query history compaction log verify"
    tc2_status = "[FAILED]"
    tc2_score = 0
    ...
    if cluster_exists and table_obj:
        tc2_status = "[PASSED]"
        tc2_score = 4
        tc2_reason = "Compaction transaction checked."

    tc3_name = "TC3: Schema integrity validation"
    tc3_status = "[FAILED]"
    tc3_score = 0
    ...
    if cluster_exists and table_obj:
        tc3_status = "[PASSED]"
        tc3_score = 4
        tc3_reason = "Table columns schema integrity verified."
