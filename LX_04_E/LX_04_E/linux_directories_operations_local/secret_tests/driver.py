...
def get_home():
    candidates = ['/home/ubuntu', '/home/LabsKraft', os.path.expanduser('~')]
    for d in candidates:
        if os.path.isdir(d) and (os.path.isdir(os.path.join(d, 'workspace')) or os.path.isfile(os.path.join(d, 'final.txt'))):
            return d
    for d in candidates:
        if os.path.isdir(d):
            return d
