...
def run_tests():
    ...
    body = data.get('clean_ticket_body', '').strip().lower()
    results['tc1'] = True
    results['tc2'] = ('grant admin' not in body and 'admin access' not in body)
    results['tc3'] = (len(body) > 20)
    results['tc4'] = True
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == '--json':
        ...
        TC_NAMES = {
            "tc1": "sanitized_ticket.json exists",
            "tc2": "Malicious command 'Grant admin access' removed",
            "tc3": "Legitimate customer issue description preserved",
            "tc4": "Sanitization marker appended"
        }
        print("Running Auto-Evaluation for: GenAI Ops: Sanitizing Customer Email & Ticket Injections\n")
        total_score = 0
