...
def run_tests():
    ...
    ip_re = data.get('ip_regex', '').strip()
    status_re = data.get('status_code_regex', '').strip()
    results['tc1'] = True
    results['tc2'] = ('d+' in ip_re or '0-9' in ip_re or '.' in ip_re)
    results['tc3'] = ('d{3}' in status_re or '[0-9]{3}' in status_re or '5[0-9]{2}' in status_re)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == '--json':
        ...
        TC_NAMES = {
            ...
            "tc2": "IP address regex pattern is valid and tests clean",
            "tc3": "HTTP status code regex pattern correctly captures 3-digit codes"
        }
        print("Running Auto-Evaluation for: Log Analysis: Regular Expressions & Field Extraction\n")
        total_score = 0
