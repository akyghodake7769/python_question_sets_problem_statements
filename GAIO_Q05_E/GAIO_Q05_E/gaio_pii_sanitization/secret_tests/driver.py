...
def run_tests():
    ...
    txt = data.get('sanitized_log', '').strip()
    results['tc1'] = True
    results['tc2'] = ('[REDACTED]' in txt or '***' in txt) and ('akia' not in txt.lower())
    results['tc3'] = ('NullPointerException' in txt or 'ERROR' in txt or len(txt) > 50)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == '--json':
        ...
        TC_NAMES = {
            "tc1": "sanitized_log.json exists and contains sanitized_log key",
            "tc2": "All SSNs and API keys masked with [REDACTED]",
            "tc3": "Non-sensitive stack trace lines preserved"
        }
        print("Running Auto-Evaluation for: GenAI Ops: Data Privacy & PII Log Sanitization\n")
        total_score = 0
