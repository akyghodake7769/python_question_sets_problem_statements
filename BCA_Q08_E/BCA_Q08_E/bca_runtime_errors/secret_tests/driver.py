...
def run_tests():
    ...
    js_path = os.path.join(get_base_path(), 'report_generator.js')
    if os.path.exists(js_path):
        with open(js_path, 'r') as f:
            code = f.read()
        results['tc1'] = True
        results['tc2'] = ('||' in code or '?' in code or 'typeof' in code)
        results['tc3'] = ('n/a' in code.lower() or 'default' in code.lower() or '?' in code)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == '--json':
        ...
        TC_NAMES = {
            "tc1": "report_generator.js is syntax-valid JS",
            "tc2": "Checks present for missing field properties",
            "tc3": "Returns fallback string when fields are null/undefined"
        }
        print("Running Auto-Evaluation for: Basic Code Analysis: Fixing Common Runtime Error Patterns\n")
        total_score = 0
