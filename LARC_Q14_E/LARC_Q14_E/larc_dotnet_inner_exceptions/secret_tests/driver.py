...
def run_tests():
    ...
    exc = data.get('inner_exception_type', '').strip().lower()
    msg = data.get('inner_exception_message', '').strip().lower()
    ln = int(data.get('failing_line_number', 0))
    results['tc1'] = True
    results['tc2'] = ('sqlexception' in exc or 'timeoutexception' in exc or 'socketexception' in exc)
    results['tc3'] = (len(msg) > 5)
    results['tc4'] = (ln == 64 or ln == 112 or ln > 0)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == '--json':
        ...
        TC_NAMES = {
            "tc1": "dotnet_exception_report.json exists",
            "tc2": "Inner exception type (SqlException/TimeoutException) identified",
            "tc3": "Inner exception message extracted correctly",
            "tc4": "Stack trace line number mapped"
        }
        print("Running Auto-Evaluation for: Log Analysis: .NET Exception Inner-Exception Chain Triage\n")
        total_score = 0
