...
def run_tests():
    ...
    svc = data.get('failing_service', '').strip().lower()
    code = int(data.get('http_status_code', 0))
    results['tc1'] = True
    results['tc2'] = ('payment' in svc or 'order' in svc or 'inventory' in svc)
    results['tc3'] = (code == 500 or code == 503 or code == 504)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == '--json':
        ...
        TC_NAMES = {
            ...
            "tc2": "Failing microservice name correctly identified",
            "tc3": "HTTP error status code correctly mapped"
        }
        print("Running Auto-Evaluation for: Log Analysis: Distributed Tracing & Correlation ID Tracking\n")
        total_score = 0
