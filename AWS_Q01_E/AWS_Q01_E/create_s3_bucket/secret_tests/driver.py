...
def verify_task(username=None, exam_code_arg=None):
    ...
    try:
        ...
        if error_code == '301':
            ...
            results['tc1'] = True
            total_score += 5
            region_error_detected = True
            detected_region = e.response.get('ResponseMetadata', {}).get('HTTPHeaders', {}).get('x-amz-bucket-region')
            print("TC1 [Bucket Creation] (5/5) - Success: Bucket found in AWS (Redirected from region mismatch).")
        elif error_code == '403':
            bucket_exists = True
            results['tc1'] = True
            total_score += 5
            print("TC1 [Bucket Creation] (5/5) - Success: Bucket found in AWS (Access restricted or region mismatch).")
        else:
            print(f"TC1 [Bucket Creation] (0/5) - Failed: Bucket not found. Error code: {error_code}")
            results['tc1'] = False
    except Exception as e:
        print(f"TC1 [Bucket Creation] (0/5) - Failed: Error checking bucket. Details: {e}")
        results['tc1'] = False
    ...
    # Get target region
    region = 'unknown'
    if region_error_detected and detected_region:
    ...
    # Instantiate regional client if needed for TC2
    eval_s3 = s3
    if region_error_detected and detected_region:
        try:
            eval_s3 = boto3.client('s3', region_name=detected_region)
        except Exception:
            pass

    # --- TEST CASE 2: PUBLIC ACCESS BLOCK ---
    try:
        pab = eval_s3.get_public_access_block(Bucket=target_bucket)
    ...
    if region in ['eu-west-1', 'eu-west-2', 'eu-west-3']:
        ...
        print(f"TC3 [Region Check] (0/5) - Failed: Found in '{region}', expected one of ['eu-west-1', 'eu-west-2', 'eu-west-3'].")
        results['tc3'] = False
