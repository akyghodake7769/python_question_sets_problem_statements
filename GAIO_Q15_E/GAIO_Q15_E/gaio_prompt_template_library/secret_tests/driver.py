...
def run_tests():
    ...
    t1 = data.get('log_review_template', '').strip()
    t2 = data.get('rca_template', '').strip()
    results['tc1'] = True
    results['tc2'] = ('{' in t1 and '}' in t1 or '{' in t2 and '}' in t2)
    results['tc3'] = (len(t1) > 10 and len(t2) > 10)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == '--json':
        ...
        TC_NAMES = {
            "tc1": "templates.json contains at least 3 templates",
            "tc2": "Templates use valid {placeholder} variable syntax",
            "tc3": "Log Review and RCA templates included"
        }
        print("Running Auto-Evaluation for: GenAI Ops: Standard Operational Prompt Template Library\n")
        total_score = 0
