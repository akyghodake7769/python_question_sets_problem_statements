...
def run_tests():
    ...
    py_path = os.path.join(get_base_path(), 'refine_prompt.py')
    if os.path.exists(py_path):
        with open(py_path, 'r') as f:
            code = f.read()
        results['tc1'] = True
        results['tc2'] = ('role' in code or 'constraint' in code or 'specificity' in code)
        results['tc3'] = ('score' in code or 'rating' in code)
        results['tc4'] = ('>=' in code or 'increase' in code or 'improvement' in code)
        results['tc5'] = ('refinement_summary.json' in code)
        results['tc6'] = True
        results['tc7'] = True
        results['tc8'] = True
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == '--json':
        ...
        TC_NAMES = {
            "tc1": "refine_prompt.py is syntax-valid Python",
            "tc2": "Evaluates prompt specificity, role assignment, and output constraints",
            "tc3": "Calculates quality score per iteration",
            "tc4": "Enforces non-decreasing score constraint across iterations",
            "tc5": "Writes refinement_summary.json",
            "tc6": "Identifies best performing prompt iteration",
            "tc7": "Logs delta improvement score",
            "tc8": "Handles single-iteration fallback gracefully"
        }
        print("Running Auto-Evaluation for: GenAI Ops: Iterative Prompt Refinement Engine\n")
        total_score = 0
