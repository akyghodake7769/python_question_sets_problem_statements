...
def test_student_code(solution_path=None):
    ...
    print("Running Local Evaluation for: Cache Tier Manager (HR-Style)\n")
    ...
    # Standardized TC Names for RT2-01 Cache Tier Manager
    TC_NAMES = [
        "Sample Case - Basic Ops",
        "Small Capacity HOT/COLD",
        "Frequency Update Check",
        "Key Replacement Logic",
        "Non-Existent Keys",
        "Multiple Puts Same Key",
        "Large K - No COLD items",
        "Capacity Edge Case (K=1)",
        "Interleaved Get & Status",
        "Status Order Consistency",
        "Volume Test - 100 Ops",
        "Volume Test - 1000 Ops",
        "Random Sequence - Small K",
        "Random Sequence - Large K",
        "Max Load Performance Test"
    ]
    ...
    for i, in_file in enumerate(test_files):
        ...
        expected_norm = "\n".join([line.strip().lower() for line in expected.splitlines() if line.strip()])
        actual_norm = "\n".join([line.strip().lower() for line in actual.splitlines() if line.strip()])
