# DevOps Lab: Linux Monitoring & Basic Troubleshooting (Local VM)

Duration : 60 Min.

## Scenario

As a DevOps engineer at LabsKraft, you are tasked with troubleshooting a local Ubuntu server/VM experiencing high resource utilization. You must perform troubleshooting tasks and generate diagnostic reports on system performance using various Linux monitoring commands.

## Task Objectives

Perform the following actions inside the default home directory (`/home/ubuntu`) of the local Ubuntu system:

### 1. CPU and Process Monitoring

- Display active processes, sort them by CPU usage, and log the snapshot output to `/home/ubuntu/cpu_monitor.txt`.

### 2. Memory Monitoring

- Check the system's memory usage in megabytes and log the output to `/home/ubuntu/memory.txt`.

### 3. Disk Diagnostics

- Check the total disk capacity and free space on all mounted filesystems and log it to `/home/ubuntu/disk.txt`.

### 4. Network Troubleshooting

- List all active network connections and listening ports on the server and log them to `/home/ubuntu/network.txt`.

## Verification

Once you have performed the tasks, you can run the verification script to check your progress and receive your score.

## Grading Criteria

Your performance will be evaluated based on the following test cases:

| Test Case | Requirement | Marks |
| --------- | ----------- | ----- |
| **TC1**   | CPU/process monitoring snapshot output `cpu_monitor.txt` generated successfully | 5 Marks |
| **TC2**   | Memory monitoring snapshot output `memory.txt` generated successfully | 5 Marks |
| **TC3**   | Disk diagnostics output `disk.txt` generated successfully | 5 Marks |
| **TC4**   | Network diagnostics output `network.txt` generated successfully | 5 Marks |

**Total Score: 20 Marks**
