...
def run_tests():
    ...
    p1 = data.get('ticket_triage_pattern', '').strip().lower()
    p2 = data.get('runbook_lookup_pattern', '').strip().lower()
    results['tc1'] = True
    results['tc2'] = ('summarization' in p1 or 'classification' in p1)
    results['tc3'] = ('retrieval' in p2 or 'rag' in p2)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == '--json':
        ...
        TC_NAMES = {
            ...
            "tc2": "Incident ticket triage mapped to Summarization/Classification",
            "tc3": "Runbook lookup mapped to Retrieval (RAG)"
        }
        print("Running Auto-Evaluation for: GenAI Ops: AI Assistance Patterns in Operations\n")
        total_score = 0
