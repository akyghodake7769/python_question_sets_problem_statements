...
def run_tests():
    ...
    eid = int(data.get('target_event_id', 0))
    lname = data.get('log_name', '').strip().lower()
    results['tc1'] = True
    results['tc2'] = (eid == 1000)
    results['tc3'] = (lname == 'application')
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == '--json':
        ...
        TC_NAMES = {
            ...
            "tc2": "Target Event ID (1000) correctly set",
            "tc3": "LogName parameter set to Application"
        }
        print("Running Auto-Evaluation for: Log Analysis: Windows PowerShell Log Filtering (Get-WinEvent)\n")
        total_score = 0
