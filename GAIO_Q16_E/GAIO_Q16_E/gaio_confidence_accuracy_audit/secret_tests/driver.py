...
def run_tests():
    ...
    p = data.get('incorrect_port_flagged')
    api = data.get('outdated_api_flagged')
    results['tc1'] = True
    results['tc2'] = (p is True or p == 'true')
    results['tc3'] = (api is True or api == 'true')
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == '--json':
        ...
        TC_NAMES = {
            "tc1": "audit_report.json exists",
            "tc2": "Incorrect port number (e.g., PostgreSQL on port 8080) flagged",
            "tc3": "Outdated API endpoint identified"
        }
        print("Running Auto-Evaluation for: GenAI Ops: LLM Confidence vs Accuracy Operational Audit\n")
        total_score = 0
