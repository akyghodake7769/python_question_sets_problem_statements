...
def run_tests():
    ...
    team = data.get('billing_service_owner_team', '').strip().lower()
    ch = data.get('slack_channel', '').strip().lower()
    results['tc1'] = True
    results['tc2'] = ('finance' in team or 'billing' in team)
    results['tc3'] = ('#team-billing' in ch or '#billing' in ch)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == '--json':
        ...
        TC_NAMES = {
            ...
            "tc2": "Correct team name mapped",
            "tc3": "Correct contact slack channel mapped"
        }
        print("Running Auto-Evaluation for: Basic Code Analysis: Service Ownership & Module Mapping\n")
        total_score = 0
