...
def run_tests():
    ...
    cmds = [c.strip().lower() for c in data.get('allowed_spl_commands', [])]
    fmt = data.get('formatting_constraint', '').strip().lower()
    results['tc1'] = True
    results['tc2'] = ('stats' in cmds or 'transaction' in cmds or 'eval' in cmds)
    results['tc3'] = ('json' in fmt or 'no markdown' in fmt or 'raw' in fmt)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == '--json':
        ...
        TC_NAMES = {
            "tc1": "spl_prompt_template.json exists",
            "tc2": "Constrains output to valid Splunk SPL commands (stats, transaction, eval)",
            "tc3": "Prohibits markdown code block wrappers if JSON requested"
        }
        print("Running Auto-Evaluation for: GenAI Ops: KQL & Splunk Query Generation Prompts\n")
        total_score = 0
