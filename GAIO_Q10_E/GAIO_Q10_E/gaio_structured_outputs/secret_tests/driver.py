...
def run_tests():
    ...
    keys = [k.strip().lower() for k in data.get('required_json_keys', [])]
    md = data.get('markdown_table_required')
    results['tc1'] = True
    results['tc2'] = ('incident_id' in keys or 'status' in keys or 'severity' in keys)
    results['tc3'] = (md is True or md == 'true')
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == '--json':
        ...
        TC_NAMES = {
            "tc1": "output_config.json exists",
            "tc2": "JSON output schema requires 'incident_id' and 'status'",
            "tc3": "Markdown table format rules specified"
        }
        print("Running Auto-Evaluation for: GenAI Ops: Structured Output Generation (JSON & Markdown Tables)\n")
        total_score = 0
