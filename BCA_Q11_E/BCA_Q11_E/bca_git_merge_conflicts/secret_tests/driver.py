...
def run_tests():
    ...
    if os.path.exists(js_path):
        with open(js_path, 'r') as f:
            code = f.read()
        results['tc1'] = ('<<<<' not in code and '====' not in code and '>>>>' not in code)
        results['tc2'] = ('function' in code or 'const' in code)
        results['tc3'] = ('staging' in code.lower() and 'production' in code.lower())
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == '--json':
        ...
        TC_NAMES = {
            "tc1": "Conflict markers are fully removed from file",
            "tc2": "Valid javascript syntax preserved",
            "tc3": "Both staging and production parameters merged successfully"
        }
        print("Running Auto-Evaluation for: Basic Code Analysis: Resolving Merge Conflicts\n")
        total_score = 0
