...
def run_tests():
    ...
    dba = data.get('dba_persona_prompt', '').strip().lower()
    sec = data.get('security_persona_prompt', '').strip().lower()
    results['tc1'] = True
    results['tc2'] = ('query' in dba or 'index' in dba or 'sql' in dba or 'lock' in dba)
    results['tc3'] = ('iam' in sec or 'policy' in sec or 'security' in sec or 'auth' in sec)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == '--json':
        ...
        TC_NAMES = {
            "tc1": "persona_prompts.json exists",
            "tc2": "DBA persona instruction includes query/index keywords",
            "tc3": "Security Engineer persona instruction includes IAM/policy keywords"
        }
        print("Running Auto-Evaluation for: GenAI Ops: Role & Persona Based Operational Prompting\n")
        total_score = 0
