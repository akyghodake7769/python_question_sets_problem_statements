...
def run_tests():
    ...
    log_default = data.get('Logging', {}).get('LogLevel', {}).get('Default', '').strip().lower()
    log_ms = data.get('Logging', {}).get('LogLevel', {}).get('Microsoft', '').strip().lower()
    results['tc1'] = True
    results['tc2'] = (log_default != '')
    results['tc3'] = (log_default == 'warning' or log_default == 'warn')
    results['tc4'] = (log_ms == 'error' or log_ms == 'err')
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == '--json':
        ...
        TC_NAMES = {
            "tc1": "appsettings.Staging.json has valid JSON",
            "tc2": "LogLevel configuration overrides configured",
            "tc3": "Default logging level set to Warning",
            "tc4": "Microsoft assembly logging set to Error"
        }
        print("Running Auto-Evaluation for: Basic Code Analysis: ASP.NET Core Environment-Specific Configuration\n")
        total_score = 0
