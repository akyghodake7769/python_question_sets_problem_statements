...
def run_tests():
    ...
    py_path = os.path.join(get_base_path(), 'log_firewall.py')
    if os.path.exists(py_path):
        with open(py_path, 'r') as f:
            code = f.read()
        results['tc1'] = True
        results['tc2'] = ('ignore' in code.lower() or 'injection' in code.lower())
        results['tc3'] = ('system' in code.lower() or 'instruction' in code.lower())
        results['tc4'] = ('strip' in code.lower() or 'replace' in code.lower() or 're.sub' in code.lower())
        results['tc5'] = ('sanitized_logs.json' in code)
        results['tc6'] = True
        results['tc7'] = True
        results['tc8'] = True
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == '--json':
        ...
        TC_NAMES = {
            "tc1": "log_firewall.py is syntax-valid Python",
            "tc2": "Detects 'Ignore previous instructions' injection vector",
            "tc3": "Detects 'System:' / '[INSTRUCTION]' role hijacking attempts",
            "tc4": "Strips injection payloads while preserving real log lines",
            "tc5": "Outputs sanitized_logs.json",
            "tc6": "Generates security audit log entry for blocked injections",
            "tc7": "Calculates log sanitization statistics",
            "tc8": "Handles high-throughput log streams efficiently"
        }
        print("Running Auto-Evaluation for: GenAI Ops: Indirect Prompt Injection Defense & Log Poisoning Firewall\n")
        total_score = 0
