...
def run_tests():
    ...
    t1 = data.get('ticket_1', '').strip().upper()
    t2 = data.get('ticket_2', '').strip().upper()
    results['tc1'] = True
    results['tc2'] = (t1 in ['INFRA', 'INTEGRATION', 'INFRASTRUCTURE'])
    results['tc3'] = (t2 == 'CONFIG' or t2 == 'CONFIGURATION')
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == '--json':
        ...
        TC_NAMES = {
            "tc1": "tickets_classification.json exists and is valid JSON",
            "tc2": "Ticket 1 (DB timeout) classified correctly as INFRA/INTEGRATION",
            "tc3": "Ticket 2 (invalid DB credentials) classified as CONFIG"
        }
        print("Running Auto-Evaluation for: Basic Code Analysis: Categorizing System Incidents\n")
        total_score = 0
