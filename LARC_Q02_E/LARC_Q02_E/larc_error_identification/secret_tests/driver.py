...
def run_tests():
    ...
    exc = data.get('root_cause_exception', '').strip().lower()
    ln = int(data.get('failing_line_number', 0))
    results['tc1'] = True
    results['tc2'] = ('nullpointerexception' in exc or 'connectexception' in exc or 'sqlexception' in exc)
    results['tc3'] = (ln == 142 or ln == 89 or ln > 0)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == '--json':
        ...
        TC_NAMES = {
            ...
            "tc2": "Root cause exception class (NullPointerException/ConnectException) correctly identified",
            "tc3": "Originating line number correctly extracted from stack frame"
        }
        print("Running Auto-Evaluation for: Log Analysis: Severity Filtering & Java Caused-By Stack Trace Triage\n")
        total_score = 0
