...
def run_tests():
    ...
    prob = data.get('is_probabilistic')
    rc = data.get('root_cause', '').strip().lower()
    temp = data.get('temperature_impact', '').strip().lower()
    results['tc1'] = True
    results['tc2'] = (prob is True or prob == 'true')
    results['tc3'] = ('token' in rc or 'confidence' in rc or 'sampling' in temp or 'temperature' in temp)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == '--json':
        ...
        TC_NAMES = {
            ...
            "tc2": "Probabilistic behavior correctly identified vs deterministic rules",
            "tc3": "Temperature / Top-P sampling parameter impact documented"
        }
        print("Running Auto-Evaluation for: GenAI Ops: Token Prediction & Probabilistic Response Concepts\n")
        total_score = 0
