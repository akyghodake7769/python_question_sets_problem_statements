...
def run_tests():
    ...
    yaml_path = os.path.join(get_base_path(), 'application.yml')
    if os.path.exists(yaml_path):
        with open(yaml_path, 'r') as f:
            code = f.read()
        results['tc1'] = True
        results['tc2'] = ('active:' in code or 'profiles:' in code)
        results['tc3'] = ('prod:' in code or 'production' in code)
        results['tc4'] = ('spring:' in code)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == '--json':
        ...
        TC_NAMES = {
            "tc1": "application.yml has valid YAML structure",
            "tc2": "spring.profiles.active variable configured correctly",
            "tc3": "Different properties defined under 'prod' divider blocks",
            "tc4": "Correct indentation spacing in configuration nodes"
        }
        print("Running Auto-Evaluation for: Basic Code Analysis: Configuring Spring Profiles in YAML format\n")
        total_score = 0
