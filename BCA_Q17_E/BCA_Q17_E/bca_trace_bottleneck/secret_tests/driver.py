...
def run_tests():
    ...
    span = data.get('slowest_span', '').strip().lower()
    dur = int(data.get('duration_ms', 0))
    results['tc1'] = True
    results['tc2'] = ('payment' in span or 'db' in span)
    results['tc3'] = (dur == 3800 or dur > 3000)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == '--json':
        ...
        TC_NAMES = {
            ...
            "tc2": "Slowest microservice span ('payment-db-query') identified",
            "tc3": "Duration (3800ms) correctly extracted"
        }
        print("Running Auto-Evaluation for: Basic Code Analysis: Distributed Trace Correlation & Latency Bottleneck\n")
        total_score = 0
