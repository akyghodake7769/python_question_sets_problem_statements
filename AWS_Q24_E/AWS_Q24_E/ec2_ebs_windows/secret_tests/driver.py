...
def verify_task():
    ...
    target_instance = f"labskraft-window-server-{username}"
    region = 'eu-west-2'
    start_time = START_TIME_STR

    print("\n" + "-" * 60)
    print(f"{'KODEBUCK AWS EC2 + EBS WINDOWS VERIFICATION':^60}")
    print("-" * 60)
    ...
    try:
        ...
        # --- TC3: EBS Volume Attached & Mounted (Formatted NTFS, F: drive) --- (5 Marks)
        tc3_passed = False
        if tc1_passed and tc2_passed:
            try:
                ...
                if is_attached:
                    ...
                    if is_managed:
                        # Run PowerShell command to verify the drive letter and filesystem type
                        cmd = "Get-Volume -DriveLetter F | Select-Object -ExpandProperty FileSystemType"
                        try:
                            ...
                            if success and output == 'NTFS':
                                tc3_passed = True
                                print(f"TC3: EBS Attached & Mounted (Drive F:) ........... [PASSED] (5/5)")
                                print(f"     └─ [Info]: Verified NTFS filesystem initialized on F: drive inside guest OS.")
                            else:
                                tc3_passed = False
                                print(f"TC3: EBS Attached & Mounted (Drive F:) ........... [FAILED] (0/5)")
                                if not success:
                                    print(f"     └─ [Reason]: Failed to run verification command in Guest OS.")
                                else:
                                    print(f"     └─ [Reason]: Volume is attached but not formatted as NTFS or not assigned to drive letter F:.")
                        except Exception as e:
                        ...
                        print(f"                   Ensure you formatted the disk as NTFS and assigned Drive Letter F:.")
                else:
                    print(f"TC3: EBS Attached & Mounted (Drive F:) ........... [FAILED] (0/5)")
