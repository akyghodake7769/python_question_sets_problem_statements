# AWS EC2 (Windows DB Server) & Advanced Storage Management

# Duration : 120 Min.

## Scenario

As a Cloud Engineer, you are tasked with provisioning compute and storage infrastructure for a database. Your objective is to create a Windows Server EC2 instance, provision a high-performance EBS volume, attach it to the instance, and initialize/format the storage using Windows Disk Management.
...
- **Instance Name:** `labskraft-window-server-<your-labskraft-username>` (replace `<your-labskraft-username>` with your actual LabsKraft username, e.g. `labs-kraft-demo106`)
...
| --------- | ------------------------------------------------------------------------------------ | ------- |
| **TC1**   | EC2 Instance Existence (`t2.micro`, Windows, named `labskraft-window-server-<username>`) | 5 Marks |
| **TC2**   | EBS Volume Created (20 GB, `io2` with 500 IOPS)                                      | 5 Marks |
| **TC3**   | EBS Volume Attached & Mounted (Formatted as `NTFS` and Assigned Drive Letter `F:`)   | 5 Marks |

**Total Score: 15 Marks**

## Important Notes

- Ensure the instance name is exactly `labskraft-window-server-<your-labskraft-username>`.
