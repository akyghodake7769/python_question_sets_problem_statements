...
def verify_task():
    ...
    try:
        ...
        job_dir = find_active_jenkins_job(["github_jenkins_maven_cicd", "Maven-CICD-Eval", "github_jenkins_maven_sonarqube"], START_TIME)
        ...
        # --- TC2: Maven Build & Unit Tests ---
        if not results.get('tc1'):
            results['tc2'] = False
            print(f"TC2: Maven Build & Unit Tests Execution ................ [FAILED] (0/10)")
            print(f"     └─ [Reason]: Prerequisite failed (Webhook invalid).")
            results['tc3'] = False
            print(f"TC3: Maven Artifact Verification (.jar/.war) ........... [FAILED] (0/10)")
                    ...
                    for b_dir in build_dirs:
                        if os.path.isdir(b_dir):
                            ...
                            if not START_TIME or b_mtime >= START_TIME:
                                ...
                                if success:
                                    tc2_passed = True
                                    latest_build_dir = b_dir
                                    break

                if tc2_passed:
                    results['tc2'] = True
                    print(f"TC2: Maven Build & Unit Tests Execution ................ [PASSED] (10/10)")
                else:
                    results['tc2'] = False
                    print(f"TC2: Maven Build & Unit Tests Execution ................ [FAILED] (0/10)")
                    print(f"     └─ [Reason]: Maven build failed or missing in current session.")
            except Exception as e:
                results['tc2'] = False
                print(f"TC2: Maven Build & Unit Tests Execution ................ [FAILED] (0/10)")
                print(f"     └─ [Error]: {str(e)}")
            
            # --- TC3: Maven Artifact Verification ---
            try:
                tc3_passed = False
                job_name = os.path.basename(job_dir)
                workspace_dir = f"/var/lib/jenkins/workspace/{job_name}"
                if not os.path.exists(workspace_dir) and os.path.exists(f"/var/snap/jenkins/common/workspace/{job_name}"):
                    workspace_dir = f"/var/snap/jenkins/common/workspace/{job_name}"
                
                if os.path.exists(workspace_dir):
                    for root, dirs, files in os.walk(workspace_dir):
                        if any(f.endswith('.jar') or f.endswith('.war') for f in files):
                            tc3_passed = True
                            break
                        if 'target' in dirs:
                ...
                if not tc3_passed:
                    log_file = os.path.join(latest_build_dir, "log") if latest_build_dir else os.path.join(job_dir, "builds/lastStableBuild/log")
                    if os.path.exists(log_file):
                        with open(log_file, 'r', encoding='utf-8', errors='ignore') as lf:
                            content = lf.read()
                            if "Building jar:" in content or "Building war:" in content or "Replacing main artifact" in content or "BUILD SUCCESS" in content or "Finished: SUCCESS" in content:
                                tc3_passed = True

                if tc3_passed:
                    results['tc3'] = True
                    print(f"TC3: Maven Artifact Verification (.jar/.war) ........... [PASSED] (10/10)")
                else:
                    results['tc3'] = False
                    print(f"TC3: Maven Artifact Verification (.jar/.war) ........... [FAILED] (0/10)")
                    print(f"     └─ [Reason]: No compiled Maven artifact (.jar/.war) found in workspace or build log.")
            except Exception as e:
                results['tc3'] = False
                print(f"TC3: Maven Artifact Verification (.jar/.war) ........... [FAILED] (0/10)")
                print(f"     └─ [Error]: {str(e)}")
