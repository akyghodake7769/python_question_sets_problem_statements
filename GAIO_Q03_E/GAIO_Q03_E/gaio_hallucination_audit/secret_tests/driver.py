...
def run_tests():
    ...
    flags = [f.strip().lower() for f in data.get('fake_flags', [])]
    cmds = [c.strip().lower() for c in data.get('hallucinated_commands', [])]
    results['tc1'] = True
    results['tc2'] = any('--force-push-all' in f for f in flags)
    results['tc3'] = any('sysclean-all' in c for c in cmds)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == '--json':
        ...
        TC_NAMES = {
            "tc1": "command_audit.json exists",
            "tc2": "Fabricated flag '--force-push-all' identified",
            "tc3": "Non-existent command 'sysclean-all' marked as hallucinated"
        }
        print("Running Auto-Evaluation for: GenAI Ops: Recognizing AI Hallucinations in CLI Commands\n")
        total_score = 0
