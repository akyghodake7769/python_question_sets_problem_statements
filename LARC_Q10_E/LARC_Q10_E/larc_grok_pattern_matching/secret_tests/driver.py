...
def run_tests():
    ...
    txt_path = os.path.join(get_base_path(), 'grok_pattern.txt')
    if os.path.exists(txt_path):
        with open(txt_path, 'r') as f:
            code = f.read()
        results['tc1'] = True
        results['tc2'] = ('%{IP' in code or '%{IPV4' in code or 'clientip' in code)
        results['tc3'] = ('status' in code and ('%{NUMBER' in code or '%{INT' in code or '%{POSINT' in code))
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == '--json':
        ...
        TC_NAMES = {
            "tc1": "grok_pattern.txt exists",
            "tc2": "%{IP:clientip} or %{IPV4:clientip} pattern included",
            "tc3": "%{NUMBER:status} or %{INT:status} pattern included"
        }
        print("Running Auto-Evaluation for: Log Analysis: Grok Pattern Field Extraction\n")
        total_score = 0
