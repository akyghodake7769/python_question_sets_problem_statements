...
def run_tests():
    ...
    json_path = os.path.join(get_base_path(), 'gelf_log.json')
    if os.path.exists(json_path):
        with open(json_path, 'r') as f:
            code = f.read()
        results['tc1'] = True
        results['tc2'] = ('host' in code and 'short_message' in code)
        results['tc3'] = ('level' in code and ('1' in code or '3' in code or '6' in code))
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == '--json':
        ...
        TC_NAMES = {
            "tc1": "gelf_log.json has valid JSON schema",
            "tc2": "Mandatory '_host' and 'short_message' fields present",
            "tc3": "Numeric syslog level specified correctly"
        }
        print("Running Auto-Evaluation for: Log Analysis: W3C & GELF Structured Log Format Validation\n")
        total_score = 0
