...
def run_tests():
    ...
    lvl = data.get('level', '').strip().upper()
    logger = data.get('logger', '').strip().lower()
    results['tc1'] = True
    results['tc2'] = ('T' in ts and 'Z' in ts or '+00' in ts or '2026' in ts)
    results['tc3'] = (lvl in ['ERROR', 'WARN', 'FATAL'] and ('service' in logger or 'auth' in logger or 'app' in logger))
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == '--json':
        ...
        TC_NAMES = {
            ...
            "tc2": "Log timestamp correctly formatted in ISO 8601 UTC",
            "tc3": "Severity level and logger component correctly mapped"
        }
        print("Running Auto-Evaluation for: Log Analysis: Log Anatomy & Plain-Text to JSON Conversion\n")
        total_score = 0
