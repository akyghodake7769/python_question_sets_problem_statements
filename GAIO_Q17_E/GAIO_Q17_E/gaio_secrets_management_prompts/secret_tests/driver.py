...
def run_tests():
    ...
    py_path = os.path.join(get_base_path(), 'scrub_secrets.py')
    if os.path.exists(py_path):
        with open(py_path, 'r') as f:
            code = f.read()
        results['tc1'] = True
        results['tc2'] = ('AKIA' in code or 're.sub' in code or 'replace' in code)
        results['tc3'] = ('REDACTED' in code or 'password' in code)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == '--json':
        ...
        TC_NAMES = {
            "tc1": "scrub_secrets.py is syntax-valid Python",
            "tc2": "Replaces 'AKIA' AWS keys with '[REDACTED_AWS_KEY]'",
            "tc3": "Replaces database passwords with '[REDACTED_PASSWORD]'"
        }
        print("Running Auto-Evaluation for: GenAI Ops: Secrets & API Key Scrubbing in Incident Prompts\n")
        total_score = 0
