...
def run_tests():
    ...
    proof = data.get('proof', '').strip().lower()
    results['tc1'] = True
    results['tc2'] = ('sre' in role or 'incident' in role or 'engineer' in role)
    results['tc3'] = ('log' in proof or 'command' in proof or 'output' in proof or 'json' in proof)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == '--json':
        ...
        TC_NAMES = {
            "tc1": "crisp_prompt.json contains all CRISP keys",
            "tc2": "Role assigned to Senior SRE / Incident Commander",
            "tc3": "Proof component requires verification command or log snippet"
        }
        print("Running Auto-Evaluation for: GenAI Ops: CRISP Prompt Framework Structuring\n")
        total_score = 0
