...
def run_tests():
    ...
    cnt = int(data.get('total_5xx_errors', 0))
    ip = data.get('top_client_ip', '').strip()
    results['tc1'] = True
    results['tc2'] = (cnt >= 12 and cnt <= 150)
    results['tc3'] = (len(ip.split('.')) == 4 or ip == '192.168.1.45')
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == '--json':
        ...
        TC_NAMES = {
            ...
            "tc2": "5xx HTTP error count correctly calculated",
            "tc3": "Top client IP address correctly identified"
        }
        print("Running Auto-Evaluation for: Log Analysis: Linux Command-Line Log Analysis (grep & awk)\n")
        total_score = 0
