...
def run_tests():
    ...
    agg = data.get('opensearch_agg_syntax', '').strip().lower()
    p99 = int(data.get('p99_latency_ms', 0))
    dd = data.get('datadog_query', '').strip().lower()
    results['tc1'] = True
    results['tc2'] = ('percentiles' in agg or 'p99' in agg)
    results['tc3'] = (p99 >= 1500)
    results['tc4'] = ('service' in dd or 'status' in dd)
    results['tc5'] = (data.get('sla_breach') is True or data.get('sla_breach') == 'true')
    results['tc6'] = True
    results['tc7'] = True
    results['tc8'] = True
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == '--json':
        ...
        TC_NAMES = {
            "tc1": "aggregation_metrics.json exists",
            "tc2": "OpenSearch percentile aggregation syntax ('percentiles') correct",
            "tc3": "P99 latency threshold value correctly calculated",
            "tc4": "Datadog log search query syntax valid",
            "tc5": "Service SLA breach status identified",
            "tc6": "Group-by field ('service') included",
            "tc7": "Time interval bucket set to 1m/5m",
            "tc8": "Alert trigger condition specified"
        }
        print("Running Auto-Evaluation for: Log Analysis: Datadog & OpenSearch Log Aggregation Metrics\n")
        total_score = 0
