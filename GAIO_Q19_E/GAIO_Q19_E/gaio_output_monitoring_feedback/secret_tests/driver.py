...
def run_tests():
    ...
    rid = data.get('flagged_recommendation_id', '').strip()
    notes = data.get('feedback_notes', '').strip()
    results['tc1'] = True
    results['tc2'] = (len(rid) > 0 and rid != 'TODO')
    results['tc3'] = (len(notes) > 10)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == '--json':
        ...
        TC_NAMES = {
            "tc1": "feedback_report.json exists",
            "tc2": "False positive recommendation ID correctly flagged",
            "tc3": "Root cause feedback notes provided"
        }
        print("Running Auto-Evaluation for: GenAI Ops: AI Output Monitoring & False Positive Feedback\n")
        total_score = 0
