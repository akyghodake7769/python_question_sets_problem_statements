...
def run_tests():
    ...
    name = data.get('dashboard_name', '').strip()
    err_p = data.get('error_rate_panel_type', '').strip().lower()
    tp_q = data.get('throughput_query', '').strip().lower()
    lat_p = data.get('latency_panel_type', '').strip().lower()
    results['tc1'] = True
    results['tc2'] = ('line' in err_p or 'timeseries' in err_p or 'chart' in err_p)
    results['tc3'] = ('count' in tp_q or 'rate' in tp_q or 'rpm' in tp_q)
    results['tc4'] = ('heatmap' in lat_p or 'bar' in lat_p or 'histogram' in lat_p)
    results['tc5'] = True
    results['tc6'] = True
    results['tc7'] = (len(data.get('auto_refresh', '')) > 0)
    results['tc8'] = True
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == '--json':
        ...
        TC_NAMES = {
            "tc1": "dashboard_config.json exists",
            "tc2": "Error rate panel query and timeseries chart configured",
            "tc3": "Throughput panel (requests per minute) configured",
            "tc4": "Latency heatmap / bar chart panel configured",
            "tc5": "Service health status gauge panel included",
            "tc6": "Alert notification channel mapped",
            "tc7": "Auto-refresh interval set to 30s/1m",
            "tc8": "Dashboard grid layout coordinates specified"
        }
        print("Running Auto-Evaluation for: Log Analysis: Dashboard Panel Visualizations & Health Metrics\n")
        total_score = 0
